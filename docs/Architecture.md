...
- **Prose RAG (17 curated JSON documents):** Visit Japan Web / entry procedures, regional pass terms, dietary venue lists. `rag_corpus/` also holds 6 markdown research notes; these are deliberately **not** in the retrieval path, because they carry team-facing hedging ("verify directly before stating it as fact") that must not reach the model as reference content.
- **Retrieval is wired into Module 1, not Module 2.** `retrieveRagDocuments()` in `server.mjs` scores each document's curated `keywords` and `title` against the user-supplied field *values* (airports, dietary requirement, pace, free-text preferences) — never the intake form's field labels, which would match "arrival" on every request and return the same documents every time. The top 4 matches are injected into Module 1's prompt with their `source_id` and `source_date` attached, and echoed back in the response as `metadata.retrievedDocuments`. Module 2 is deliberately excluded: its prompt forbids new numbers and requires every `source_id` to be copied from the calculator's own `evidence` array, so injecting a document corpus there would break the Program-of-Thoughts boundary that the Pass ROI Auditor's credibility rests on.
- **Timing disclosure — the Stage 6 evaluation predates this wiring.** The 60-run Stage 6 evaluation was executed on 7 Sept 2026 against a build in which `rag_corpus/` was curated but never loaded. Retrieval was wired on 9 Sept 2026 after the gap was found. Every Stage 6 and Stage 7 number in this repository therefore describes the **pre-retrieval** build, and re-running the evaluation against the current build is an open item, not a completed one. No Stage 6 artefact has been edited to suggest otherwise.
