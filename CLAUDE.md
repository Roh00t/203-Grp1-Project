...
- **No vector database.** Retrieval over the 17 curated JSON documents is rule-based keyword matching — deliberately chosen because the corpus is exact-match jargon (station names, pass names, regulation terms), not because of time pressure alone. Wired into **Module 1 only** (`retrieveRagDocuments()` in `server.mjs`, 9 Sept 2026). Do not inject the corpus into Module 2: its prompt forbids new numbers and requires every `source_id` to be copied from the calculator's `evidence`, so a document corpus there would break the Program-of-Thoughts boundary. The 6 markdown files in `rag_corpus/` are research notes and stay out of the retrieval path.
...
- **No silent defaults for missing data — anywhere in the pipeline.** This generalises the rule above and is the single invariant the system's evidence-led claims rest on. Three concrete cases, all already implemented, none of which may be "fixed" by supplying a fallback value:
  - An **unmatched dietary venue** resolves to `unverified`. Not compliant.
  - A **missing geographic ward pair** resolves to `unverified` — including a same-ward pair for an uncurated area (e.g. `Fujiyoshida` ↔ `Fujiyoshida`). Do **not** add a baseline walking time for same-ward pairs in code. Curated areas already carry explicit same-ward rows in `data/travel_time_table.json`; an area with no row is an area the curated data knows nothing about, and defaulting it would silently pass a location nobody verified. The fix for a real gap is a sourced row from Ulfa, not a constant in the validator.
  - An **unpriced fare segment** (a station outside the curated 19-station matrix, e.g. `Mishima -> Kyoto`) raises `MissingFareError`, the audit is withheld, and the UI degrades gracefully to "Exact pricing unavailable for this route". Do **not** substitute a dummy price, an average, or a nearest-station estimate.
  
  **Why this is locked:** a system that quietly fills gaps produces a verdict that looks identical whether or not it was grounded, which destroys the distinction the whole project rests on. The strict failure state is the evidence. It is what makes "≥90% geographic plausibility with failures flagged, never silently shown as fine" and the Faithfulness metric defensible under direct questioning — and it is what allowed Stage 7 to attribute failures to real causes rather than to a fallback masking them. Presentation may soften (see the UI remediation in `docs/Architecture.md` Stage 5); the underlying state must not.
...
- **Deployed** at <https://hellobird.io/PE6203/index.html> (verified 10 Sept 2026). Static hosting: demo mode and the deterministic checks work live; `/api/generate` returns 404 there, so live Gemini generation and the Stage 6 harness still require `npm start` locally. The deployed build predates the 9 Sept UI remediation — redeploy to pick it up.
